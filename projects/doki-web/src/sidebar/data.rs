...
impl SideNavGroupItem {
    pub fn item<S>(name: S, link: S, icon: Option<SideNavItemIcon>) -> Self where S: Into<String> {
        SideNavGroupItem::Simple(SideNavItemSimple {
            icon,
            name: name.into(),
            link: link.into(),
        })
    }
    pub fn list<S>(title: S, list: Vec<SideNavItemList>, closed: (bool, bool)) -> Self where S: Into<String> {
