...
fn test2() {
    ...
    assert_eq!(url, Url::parse("https://localhost/config?language=en-US").unwrap());
    // version level
    let url = ver.get_link(&url, "v1.0.0-alpha").unwrap();
    assert_eq!(url, Url::parse("https://localhost/config?language=en-US&version=v1.0.0-alpha").unwrap());
    // section level
    let url = sidebar.get_link(&url).unwrap();
    assert_eq!(url, Url::parse("https://localhost/config/section?language=en-US&version=v1.0.0-alpha").unwrap());
    // group level
    ...
    assert_eq!("/config?language=en-US", url)
}
