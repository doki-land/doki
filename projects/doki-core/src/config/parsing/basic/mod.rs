...
pub(crate) fn parse_bool(root: &HashMap<String, Value>, key: &str) -> Option<bool> {
    get_normalized_object(root, key)?.clone().into_bool().ok()
}

#[inline]

pub(crate) fn parse_string(root: &HashMap<String, Value>, key: &str) -> Option<String> {
    get_normalized_object(root, key)?.clone().into_string().ok()
}

#[inline]

pub(crate) fn parse_object(root: &HashMap<String, Value>, key: &str) -> Option<HashMap<String, Value>> {
...
pub(crate) fn parse_string_list(root: &HashMap<String, Value>, key: &str) -> Option<Vec<String>> {
...
pub(crate) fn get_normalized_object<'a>(root: &'a HashMap<String, Value>, key: &str) -> Option<&'a Value> {
...
pub(crate) fn load_config_string(input: &str, format: FileFormat) -> Config {
    Config::builder().add_source(File::from_str(input, format)).build().unwrap()
}
pub(crate) fn load_config_file(dir: &Path, name: &str) -> Result<Value> {
